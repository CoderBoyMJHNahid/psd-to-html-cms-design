...
class conexioni
{
    ...
    public function log_db_errors( $error, $query )
    {
        ...
        $headers .= 'To: Admin <ramon.p@marmo.com.mx>' . "\r\n";
        $headers .= 'From: SISAD <sistemas@fabrica.sistemasmarmo.net>' . "\r\n";
    ...
    public function __construct()
    {
	    ...
	    $connection_hostname = $hostdb;            // Host name
        $connection_username = $usrdb;         // User name
        $connection_password = $pswdb;          // Password
        $connection_database = $nomdb;          // Data Base
        
        mb_internal_encoding( 'UTF-8' );
        mb_regex_encoding( 'UTF-8' );
            $this->link = new mysqli( $connection_hostname, $connection_username, $connection_password, $connection_database );
    ...
    public function db_common( $value = '' )
    {
        if( is_array( $value ) )
        {
            foreach( $value as $v )
            {
                if( preg_match('/AES_DECRYPT/i', $v) || preg_match('/AES_ENCRYPT/i', $v) || preg_match('/NOW()/i', $v) || preg_match( '/SHA1/i', $v ))
                {
                    return true;
                }
                else
        ...
        {
            if( preg_match( '/AES_DECRYPT/i', $value ) || preg_match( '/AES_ENCRYPT/i', $value ) || preg_match( '/NOW()/i', $value ) || preg_match( '/SHA1/i', $v ) )
            {
                return true;
            }
        }
    ...
    public function exists( $table = '', $check_val = '', $params = array() )
    {
        ...
        foreach( $params as $field => $value )
        {
            if( !empty( $field ) && !empty( $value ) )
            {
                ...
                if( $this->db_common( $value ) || strpos($value,"|FUNC|")!==FALSE )
                {
    ...
    */
    ...
    public function tracking( $mod, $usr )
    {
         $sql = $this->lastsql;

         $sql = $this->filter($sql);

        $track="INSERT INTO `tb_track` (`id`, `modulo`, `accion`, `usuario`, `fecha`) 
                VALUES (NULL, '$mod', '$sql', '$usr', NOW());";

        $this->link->query( $track );
    }
